...
PEOPLE_IMAGE_PATH = os.path.join(SCRIPT_DIR, "test_files", "people_detection.png")
...
D_MODEL_PATH, D_MODEL_PROC_PATH = get_model_path(D_MODEL_NAME), get_model_proc_path(D_MODEL_NAME)
C1_MODEL_NAME = "person-attributes-recognition-crossroad-0230"
C1_MODEL_PATH, C1_MODEL_PROC_PATH = get_model_path(C1_MODEL_NAME), get_model_proc_path(
    C1_MODEL_NAME
)
C2_MODEL_NAME = "vehicle-attributes-recognition-barrier-0039"
C2_MODEL_PATH, C2_MODEL_PROC_PATH = get_model_path(C2_MODEL_NAME), get_model_proc_path(
    C2_MODEL_NAME
)
...
PEOPLE_GOLD_TRUE = [
    BBox(
        ...
        0.6875,
        0.9851485148514851,
        [{"label": "M: has_longpants ", "layer_name": "453", "name": "person-attributes"}],
        tracker_id=1,
        class_id=1,
    ),
    BBox(
        ...
        0.8305555555555556,
        1.0,
        [{"label": "F: has_bag has_longhair ", "layer_name": "453", "name": "person-attributes"}],
        tracker_id=2,
        class_id=1,
    ),
]
...
CAR_GOLD_TRUE = [
    BBox(
        ...
        1.0,
        [
            {"label": "white", "layer_name": "color", "name": "color"},
            {"label": "car", "layer_name": "type", "name": "type"},
        ],
        tracker_id=1,
        class_id=2,
    ),
    BBox(
        ...
        0.9467592592592593,
        [
            {"label": "red", "layer_name": "color", "name": "color"},
            {"label": "car", "layer_name": "type", "name": "type"},
        ],
        tracker_id=2,
        class_id=2,
...
class TestVehiclePedestrianTracker(unittest.TestCase):
    ...
    def test_vehicle_tracker_pipeline(self):
        pipeline_runner = TestPipelineRunner()
        for pipeline_str in set_of_pipelines():
            pipeline_runner.set_pipeline(pipeline_str, CAR_IMAGE_PATH, CAR_GOLD_TRUE, True)
            pipeline_runner.run_pipeline()
